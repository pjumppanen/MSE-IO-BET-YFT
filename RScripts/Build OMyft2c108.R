...
# --- Description --------------------------------------------------------------
OMd@Name          <- "OMyft2c108"
OMd@Label         <- "OMyft2c108" #useful for chaning graphics labels
OMd@Date          <- "Jun 2016"
OMd@Author        <- "D.Kolody"
OMd@Notes         <- "OM based on 54 SS specifications with stochastic variability; 108 reps"
...
OMd@SSRootDir     <- "./OMconditioning/YFT/gridY3/"          # Root dir for SS results outputs
OMd@SBlim         <- 0.4
OMd@Flim          <- 1.4

#Suite of 52 OMs from a balanced 3X3X3x2 grid
source("./OMconditioning/RStuff/makeGridY3.f.R")
gridY3List <- makeGridY3.f(makeGrid=F)
#OMd@OMList        <- as.list(gridY3List[c(2,51)])
OMd@OMList        <- as.list(gridY3List)
OMd@nsimPerOMFile <- array(rep(2,length(OMd@OMList)),dim=length(OMd@OMList))   # Number of simulations per each SS specification file (vector of length OMList allows differental weighting, i.e. c(10,50,25)
...
# --- save object --------------------------------------------------------------
save(OMd,file=paste(getwd(),"/Objects/OMd.OMyft2c108.RDA",sep=""))
