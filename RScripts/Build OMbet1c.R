...
# --- Description --------------------------------------------------------------
OMd@Name          <- "OMbet1c"
OMd@Label         <- "OMbet1c" #useful for changing graphics labels
OMd@Date          <- "Jun 2016"
OMd@Author        <- "D.Kolody"
OMd@Notes         <- "OM based on 18 SS specifications with no stochastic variability; 2160 reps"
...
OMd@nsimPerOMFile <- array(rep(120,length(OMd@OMList)),dim=length(OMd@OMList))   # Number of simulations per each SS specification file (vector of length OMList allows differental weighting, i.e. c(10,50,25)
...
# --- save object --------------------------------------------------------------
save(OMd,file=paste(getwd(),"/Objects/OMd.OMbet1c.RDA",sep=""))
