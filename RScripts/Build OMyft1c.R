...
# --- Description --------------------------------------------------------------
OMd@Name          <- "OMyft1c"
OMd@Label         <- "OMyft1c" #useful for changing graphics labels
OMd@Date          <- "Jun 2016"
OMd@Author        <- "D.Kolody"
OMd@Notes         <- "OM based on 36 SS specifications with stochastic variability; 2160 reps"
OMd@PrimarySource <- "Substantive YFT Demo case"
OMd@CppMethod     <- 1 # 1 = Use C++ Baranov solution, 0 = Use R based Popes approximation
OMd@UseCluster    <- 1 # 1 = Use cluster of R processes, 0 = Use single R process
...
OMd@SSRootDir     <- "./OMconditioning/YFT/gridY2/"          # Root dir for SS results outputs
OMd@SBlim         <- 0.4
OMd@Flim          <- 1.4

#Suite of 36 OMs from a balanced 3X3X3x2 grid
source("./OMconditioning/RStuff/makeGridY2.f.R")
gridY2List <- makeGridY2.f(makeGrid=F)
#OMd@OMList        <- as.list(gridY2List[c(2,35)])
OMd@OMList        <- as.list(gridY2List)
OMd@nsimPerOMFile <- array(rep(60,length(OMd@OMList)),dim=length(OMd@OMList))   # Number of simulations per each SS specification file (vector of length OMList allows differental weighting, i.e. c(10,50,25)
OMd@proyears      <- as.integer(26)              # Number projection years
...
OMd@lastCalendarYr <- as.integer(2014)   # used to convert to actual years
OMd@firstMPYr      <- as.integer(2018)   #MP kicks in here (projections in the intervening period may require an observed catch input to handle data lags)
...
OMd@catchBridge    <- as.karray(c(300000))    # known catch history between last assessment year in OM and firstMPYr (length of min 0 to max firstMPYr-lastCalendarYr-1)
...
# --- save object --------------------------------------------------------------
save(OMd,file=paste(getwd(),"/Objects/OMd.OMyft1c.RDA",sep=""))
