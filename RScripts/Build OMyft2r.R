...
# --- Description --------------------------------------------------------------
OMd@Name          <- "OMyft2r"
OMd@Label         <- "OMyft2r" #useful for chaning graphics labels
OMd@Date          <- "Jun 2016"
OMd@Author        <- "D.Kolody"
OMd@Notes         <- "OM based on 54 SS specifications with stochastic variability; 2160 reps"
...
OMd@nsimPerOMFile <- array(rep(40,length(OMd@OMList)),dim=length(OMd@OMList))   # Number of simulations per each SS specification file (vector of length OMList allows differental weighting, i.e. c(10,50,25)
...
# --- save object --------------------------------------------------------------
save(OMd,file=paste(getwd(),"/Objects/OMd.OMyft2r.RDA",sep=""))
