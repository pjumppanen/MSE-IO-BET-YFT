...
# --- Description --------------------------------------------------------------
OMd@Name          <- "OMbetNEr"
OMd@Label         <- "OMbetNEr" #useful for changing graphics labels
OMd@Date          <- "Jun 2016"
OMd@Author        <- "D.Kolody"
OMd@Notes         <- "Low error, no stochastic variability 18 SS model ensemble specification for testing"
OMd@PrimarySource <- "test model using R projections"
OMd@CppMethod     <- 0 # 1 = Use C++ Baranov solution, 0 = Use R based Baranov approximation
...
OMd@OMList        <- as.list(gridB1List[c(6,13)]) #[c(1,2,17,18)]) #("R1P1_h70_M06_q0Mhigh") #gridB1List[c(1,18)])
...
# --- save object --------------------------------------------------------------
save(OMd,file=paste(getwd(),"/Objects/OMd.OMbetNEr.RDA",sep=""))
