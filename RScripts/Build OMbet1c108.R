...
# --- Description --------------------------------------------------------------
OMd@Name          <- "OMbet1c108"
OMd@Label         <- "OMbet1c108" #useful for changing graphics labels
OMd@Date          <- "Jun 2016"
OMd@Author        <- "D.Kolody"
OMd@Notes         <- "OM based on 18 SS specifications with no stochastic variability; 108 reps"
OMd@PrimarySource <- "arbitrary specification for testing"
...
OMd@OMList        <- as.list(gridB1List)
OMd@nsimPerOMFile <- array(rep(6,length(OMd@OMList)),dim=length(OMd@OMList))   # Number of simulations per each SS specification file (vector of length OMList allows differental weighting, i.e. c(10,50,25)
OMd@proyears      <- as.integer(27)              # Number projection years
...
#OMd@catchBridge    <- as.karray(c(0.001,0.001,0.001,0.001,0.001))    # known catch history between last assessment year in OM and firstMPYr (length of min 0 to max firstMPYr-lastCalendarYr-1)
...
OMd@selExpRange  <- 0.6             # sel temporal variability exponent - oscillates with a sin wave rangeing between +/- exp(selExpRange)
OMd@selAgeRange  <- 1.               # 0=no age shift, 2 means (discretized) sine wave shift of sel vector between - 2 and + 2 age class
...
# --- save object --------------------------------------------------------------
save(OMd,file=paste(getwd(),"/Objects/OMd.OMbet1c108.RDA",sep=""))
