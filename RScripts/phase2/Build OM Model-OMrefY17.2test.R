...
# --- Description --------------------------------------------------------------
OMd@Name          <- "yftY17.2test"
OMd@Label         <- "yftY17.2test" #useful for changing graphics labels
OMd@Date          <- "Dec2017"
OMd@Author        <- "D.Kolody"
OMd@Notes         <- "test YFT reference OM based on bivariate B/BMSY and MSY sampling of OMref17.2grid"
OMd@PrimarySource <- "test only"
...
OMd@SSRootDir     <- "../OMconditioning/YFT/gridY17.2/"          # Root dir for SS results outputs
OMd@SBlim         <- 0.4
OMd@Flim          <- 1.4
...
load("objects\\OMref17.2List.RDA")       #the weighted list of models

OMd@OMList       <- as.list(names(OMref17.2List[c(1:3)]))
OMd@modelWeight  <- as.karray(OMref17.2List[c(1:3)])  #= "karray",    # Plausibility weighting for given OMFile
OMd@totalSims    <- 20                 # = "numeric",   # Total number of simulations in MSE run.
# nSimPerOMFile should now be automatically oweritten based on totalSims and modelWeight (?)
OMd@nsimPerOMFile <- array(rep(10,length(OMd@OMList)),dim=length(OMd@OMList))   # Number of simulations per each SS specification file (vector of length OMList allows differental weighting, i.e. c(10,50,25)
