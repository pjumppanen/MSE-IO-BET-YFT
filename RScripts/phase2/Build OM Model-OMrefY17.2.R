...
# --- Description --------------------------------------------------------------
OMd@Name          <- "yftY17.2"
OMd@Label         <- "yftY17.2" #useful for changing graphics labels
OMd@Date          <- "Dec2017"
OMd@Author        <- "D.Kolody"
OMd@Notes         <- "YFT reference OM based on bivariate B/BMSY and MSY sampling of OMref17.2grid"
OMd@PrimarySource <- "see working paper from 2018 IOTC MWG informal MSE mtg."
...
OMd@OMList       <- as.list(names(OMref17.2List))
modelWeight      <- as.numeric(OMref17.2List)  #= "karray",    # Plausibility weighting for given OMFile
totalSims        <- 2000                 # = "numeric",   # Total number of simulations in MSE run.
