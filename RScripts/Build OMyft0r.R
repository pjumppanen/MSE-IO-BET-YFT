...
# --- Description --------------------------------------------------------------
OMd@Name          <- "OMyft0r"
OMd@Label         <- "OMyft0r" #useful for changing graphics labels
OMd@Date          <- "Jun 2016"
OMd@Author        <- "D.Kolody"
OMd@Notes         <- "OM based on 36 SS specifications without stochastic variability; 1 rep per SS configuration, R projections"
OMd@PrimarySource <- "YFT Test case"
...
OMd@catchBridge    <- as.karray(c(1))    # known catch history between last assessment year in OM and firstMPYr (length of min 0 to max firstMPYr-lastCalendarYr-1)
...
OMd@selExpRange  <- 0.     #0.6  0           # sel temporal variability exponent - oscillates with a sin wave rangeing between +/- exp(selExpRange)
OMd@selAgeRange  <- 0.      #3.   0           # 0=no age shift, 2 means (discretized) sine wave shift of sel vector between - 2 and + 2 age class
...
OMd@Ccv    <-c(0., 0.0001)
OMd@Icv    <-c(0., 0.0001)  #c(0., 0.0001) # c(0.3, 0.3001)
OMd@Dcv    <-c(0., 0.0001)
OMd@Btcv   <-c(0., 0.0001)
OMd@Ftcv   <-c(0., 0.0001)
OMd@Cbcv   <- 0.
...
# --- save object --------------------------------------------------------------
save(OMd,file=paste(getwd(),"/Objects/OMd.OMyft0r.RDA",sep=""))
