...
setClass("OMd",representation(
               ...
               #tunePMTarget Definition seems wrong, i.e.
             ...
             prototype = list(
               tuneTol       = 0.01,
               tuneLogDomain = c(-3,0.5),      # -3, 0.5; maybe should transform relevant MP paramters so this domain can remain unchanged
               RecScale      = karray(c(1)),
...
setClass("OMss",representation(
              ...
              FAgeRange       = "karray",  # Age range for Frep calculations
...
setMethod("initialize", "OMss", function(.Object,OMd, Report=F, UseMSYss=0)
{
  ...
  .Object@OMid          <- karray(rep(NA, sum(OMd@totalSims), dim=1))
  ...
  if(OMd@totalSims == 0) # define objects with the original nsimPerOMFile
  {
    .Object@nsim       <- as.integer(sum(.Object@nsimPerOMFile))
  } else {
    .Object@nsim       <- as.integer(sum(OMd@totalSims))
  }
  ...
  else if (length(OMd@modelWeight) == length(OMd@OMList))
  {
    ...
    .Object@nsimPerOMFile <- karray(as.vector(t(rmultinom(1, size=OMd@totalSims-1, prob=OMd@modelWeight))))
    ...
    #{
      ...
      if (lastSimNum >= firstSimNum)
      {
        ...
        if (iom == 1)
        {
          ...
          .Object@FAgeRange    <- karray(as.double(NA), dim=c(2))
        ...
        .Object@FAgeRange <- karray(tmp[!is.na(tmp)], dim=2) + 1 #  MSE columns add 1 because column 1 is SS age 0
        rm(tmp)
        ...
        if (iom == 1)
        {
          ...
          for (i in 1:nrow(ssMod$cpue))
          {
            ...
            .Object@CPUEobsMR[,yrSeas[1]:.Object@nyears,yrSeas[2]:.Object@nsubyears, as.numeric(substr(ssMod$cpue$Name[i], 7, 7))] <- ssMod$cpue$Obs[i]
          }
      ...
      #.Object@mov[firstSimNum:lastSimNum,,,,,] <- mov*0+0.25 #uniform mixing test
        ...
        a1 <- .Object@FAgeRange[1]
        a2 <- .Object@FAgeRange[2]
...
runProjection <- function(.Object, OM, projSims, CPUEobsR, TACEErrorAll, MPs, interval, Report, CppMethod, UseCluster, EffortCeiling, TACTime, rULim, tune)
{
  ...
  nMPs      <- length(MPs)
  FAgeRange <- c(OM@FAgeRange[1], OM@FAgeRange[2])
...
setMethod("initialize", "MSE", function(.Object, OM, MPs, interval=3, Report=F, CppMethod=NA, UseCluster=NA, EffortCeiling = as.double(20.0), TACTime = 0.5, rULim=0.5)
{
  ...
  if (HasTuning && (length(tuned_MPs) > 0))
  {
    for (idx in tuned_MPs_Idx)
    {
      ...
      opt_fn <- function(tuneLog10, bisect=F)
      {
        ...
        tuneError <- abs(OM@tunePMTarget - tuneValue)/OM@tunePMTarget
        #tuneError <- abs(OM@tunePMTarget - tuneValue)
        #tuneError <- (OM@tunePMTarget - tuneValue)^2

        if(bisect){
          print(paste("bisect - target:", OM@tunePMTarget, ", Current value:", tuneValue, ", tuning error:", tuneError, ", tune (MP-scale Par value): ", tune))
          return (tuneValue)
        } else {
          print(paste("target:", OM@tunePMTarget, ", Current value:", tuneValue, ", tuning error:", tuneError, ", tune (MP-scale Par value): ", tune))
          return (tuneError)
        }
      }
      ...
      if(bisect){  #custom bisection solver for percentile tuning
        ...
        if((loVal < OM@tunePMTarget & hiVal < OM@tunePMTarget & tmpVal < OM@tunePMTarget) |
           (loVal > OM@tunePMTarget & hiVal > OM@tunePMTarget & tmpVal > OM@tunePMTarget)){
...
findFrep <- function(NsoRbySPAM, M_Y, nsim, npop, nages, nsubyears,FAgeRange)
{
  ...
  Frep       <- apply(FsoRbySPAM[,,FAgeRange[1]:FAgeRange[2],], mean, MARGIN=c(1))  # 2:27 = true ages 1:26 (1:26 used by SS)
