...
makeGridY17.2Tag.f <- function (path='H:\\C-offline\\MSE-IO-BET-YFT\\OMconditioning\\YFT\\gridY17.2Tag', makeGrid=T, splitMasterBatch=T, doHess=T,
#makeGridY17.2Tag.f <- function (path='C:\\MSE-IO-BET-YFT\\OMconditioning\\YFT\\gridY17.2Tag', makeGrid=T, splitMasterBatch=T, doHess=T,
  ...
  sigR.val = c( "sr6"),  # sigmaR
  M.val    = c( "M10", "M08","M06"), #mort
  t.val    = c("t10"),   #tag weight
  q.val    = c("q0","q1"),           #CPUE q trend % per y
  mix.val  = c("x3","x8"),            #tag mixing period (qtrs); irrelevant oif tags not used
...
makeGridY17.2Tag.f(doHess=F, makeGrid=F)